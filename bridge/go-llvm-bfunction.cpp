    ...
    case ParmDirect: {
      ...
      if (paramInfo.abiTypes().size() == 1) {
        ...
        for (unsigned i = 0; i < paramInfo.abiTypes().size(); ++i) {
          std::string argp(name);
          argp += ".chunk" + std::to_string(i);
          arguments_[soff+int(i)]->setName(argp);
        }
      }
                                ...
                                llvm::Value *sploc)
{
  ...
  for (unsigned i = 0; i < paramInfo.abiTypes().size(); ++i) {
    std::string tag1(namegen("field" + std::to_string(i)));
    llvm::Value *fieldgep =
        builder.CreateConstInBoundsGEP2_32(llst, bitcast, 0, i, tag1);
    llvm::Value *argChunk = arguments_[paramInfo.sigOffset() + int(i)];
