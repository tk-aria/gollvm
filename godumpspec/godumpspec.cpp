...
class GoDumpHelper {
 ...
 private:
  ...
  std::unordered_set<uint32_t> anonUnions_;
...
void GoDumpHelper::visitType(const DWARFDie &die)
{
  ...
  //std::cerr << "visit offset " << die.getOffset() << "\n";
...
bool GoDumpHelper::generateMember(const DWARFDie &die)
{
  ...
  if (!name) {
    ...
    anonUnions_.insert(ctyp.getOffset());
    anonSub = true;
  } else {
    assert(name);
    if (isGoKeyWord(name))
...
{
  bool rval = true;
  if (anonUnions_.find(die.getOffset()) == anonUnions_.end())
    buf() << "struct { ";
...
int GoDumpHelper::visitMacro(const std::string &line, unsigned lno)
{
  ...
  //std::cerr << "mac name :" << macname << ":\n";
  //std::cerr << "mac body :" << macbody << ":\n";
  return 0;
}
