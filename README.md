...
You'll need to have an up-to-date copy of cmake on your system (3.6 vintage) to build Gollvm.
...
```
 % go build -ldflags -static-libgo myprogram.go
```

Note that this will increase binary size dramatically. It is probably a good idea to include the options "-ffunction-sections -fdata-sections -Wl,--gc-sections" to ameliorate the effects of the size increase.
...
```
% go build -gccgoflags -gz mumble.go
```
...
## Can I use FDO or Thin LTO with gollvm?  <a name="thinltofdo"></a>
