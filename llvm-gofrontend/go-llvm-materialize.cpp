...
class VarContextVisitor {
 ...
 private:
  ...
  Bexpression *memArg(Bexpression *expr) {
    ...
    if (expr->flavor() == N_StructField ||
        expr->flavor() == N_ArrayIndex ||
        expr->flavor() == N_Address ||
        expr->flavor() == N_Deref ||
        expr->flavor() == N_Conversion ||
        expr->flavor() == N_PointerOffset)
      return kids[0]->castToBexpression();
    return nullptr;
  }

  bool isMem(Bexpression *expr) {
    return (expr->flavor() == N_Var ||
            expr->flavor() == N_StructField ||
            expr->flavor() == N_Address ||
            expr->flavor() == N_Deref ||
            expr->flavor() == N_Conversion ||
            expr->flavor() == N_ArrayIndex ||
