...
Btype *TypeManager::synthesizeNonZeroType(Btype *typ, Bexpression *one)
{
  if (typeSize(typ) != 0)
    return typ;
  switch(typ->flavor()) {
    case Btype::ArrayT: {
      ...
      Btype *elemtyp = synthesizeNonZeroType(bat->elemType(), one);
    ...
    case Btype::StructT: {
      ...
      if (fields.size()) {
        ...
        for (unsigned i = 0; i < fields.size(); ++i)
          nzfields[i].btype = synthesizeNonZeroType(fields[i].btype, one);
