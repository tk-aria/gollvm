...
// to indicate this is a Go function.
...
static AttributeList legalizeCallAttributes(AttributeList AL) {
  ...
  for (unsigned i = AttributeList::FirstArgIndex, e = AL.getNumAttrSets(); i < e; ++i) {
    AttrBuilder ArgAttr = AL.getAttributes(i);
    //ArgAttr.removeAttribute(Attribute::StructRet);
    //ArgAttr.removeAttribute(Attribute::ByVal);
    Ret = Ret.addAttributes(Ctx, i+5, ArgAttr);
  }
...
attachStackMap(uint64_t StatepointID, Instruction *LandingPad) {
  ...
  std::string Name = (Twine("go..stackmap.") + Twine(StatepointID)).str();
  Constant *C = M->getOrInsertGlobal(Name, Type::getInt64Ty(M->getContext()));
  LandingPad->setOperand(0, C);
}

static bool hasPointer(Type *T);
static void getPtrBitmapForType(Type *T, const DataLayout &DL, SmallVectorImpl<Value *> &Words);
...
zeroAmbiguouslyLiveSlots(Function &F, SetVector<Value *> &ToZero,
  ...
  for (Instruction &I : F.getEntryBlock())
    if (ToZero.count(&I) != 0) {
      IRBuilder<> Builder(I.getNextNode());
      Value *Zero = Constant::getNullValue(I.getType()->getPointerElementType());
...
bool GoStatepoints::runOnFunction(Function &F, DominatorTree &DT,
  ...
  // Make noreturn call's successor unreachable.
  // This is needed in the following case:
  //
  // if (bound check failed) {
  //   panic()
  // } else {
  //   tmp = ...
  // }
  // use tmp
  //
  // tmp should only be live in else-block and below. Propagating backwards
  // to then-block causes problem.
  // TODO: consider if this is simpler to do in the frontend?
  for (BasicBlock &BB : F) {
    for (Instruction &I : BB)
      if (CallInst *CI = dyn_cast<CallInst>(&I))
        if (Function *Fn = CI->getCalledFunction())
          if (Fn->doesNotReturn()) {
            MadeChange = true;
            changeToUnreachable(I.getNextNode(), false, false, &DTU);
            break;
          }
    ...
    if (InvokeInst *II = dyn_cast<InvokeInst>(BB.getTerminator()))
      if (Function *Fn = II->getCalledFunction())
        if (Fn->doesNotReturn()) {
          MadeChange = true;
          BasicBlock *ContBB = II->getNormalDest();
          if (ContBB->getSinglePredecessor())
            changeToUnreachable(&*ContBB->begin(), false, false, &DTU);
          else {
            BasicBlock *New = BasicBlock::Create(F.getContext(), "dead", &F);
            new UnreachableInst(New->getContext(), New);
            RemovePredecessorAndSimplify(ContBB, &BB);
            II->setNormalDest(New);
            DTU.deleteEdge(&BB, ContBB);
            DTU.insertEdge(&BB, New);
          }
        }
  }
...
static void
getPtrBitmapForType(Type *T, const DataLayout &DL, SmallVectorImpl<Value *> &Words) {
