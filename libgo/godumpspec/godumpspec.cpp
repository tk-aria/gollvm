...
class MacroParser {
 public:
  ...
  int postProcessMacros();
  void emitMacros(raw_ostream &os,
                  const std::unordered_set<std::string> &emittedTypeNames);

 private:
  // raw (unexpanded) bodies
  std::vector<std::string> names_;
  std::vector<std::string> bodies_;
  // cooked (expanded) macro bodies
  std::vector<std::string> cooked_;
  // visit state for each macro (by index)
  std::unordered_map<unsigned, MacVisitState> visited_;
};
...
//
class GoDumpHelper : protected MacroParser {
 public:
  explicit GoDumpHelper(raw_ostream &os);
  void readDwarf(DWARFCompileUnit *cu);
  int visitMacro(const std::string &line, unsigned lno);
  void emit();
 ...
 private:
  ...
  // Go source code directly to the current buffer if emission is
  // enabled.
...
void GoDumpHelper::readDwarf(DWARFCompileUnit *cu)
{
  ...
  for (const auto &entry : cu_->dies()) {
    ...
    // queue only named types at the top level
    if (isType(die.getTag()) && dieName(die) != nullptr)
      enqueueType(die);
...
void GoDumpHelper::visitType(const DWARFDie &die)
{
  ...
  if (emit_) {
    ...
    if (ok) {
      ...
      // emit enum literals
      if (enumLiterals_.size()) {
        for (auto lit : enumLiterals_) {
          if (emittedTypeNames_.find(lit.first) != emittedTypeNames_.end())
            continue;
          os_ << "const _" << lit.first << " = " << lit.second << "\n";
          emittedTypeNames_.insert(lit.first);
        }
      }
    }
  }
...
int GoDumpHelper::visitMacro(const std::string &line, unsigned lno)
{
  return visitMacroLine(line, lno);
}
...
int MacroParser::visitMacroLine(const std::string &line, unsigned lno)
{
  ...
  if (std::strncmp(line.c_str(), "#define ", 8) != 0) {
    errs() << "malformed macro input at line " << lno << ": " << line << "\n";
    return 1;
  }
  ...
  unsigned nameIdx = names_.size();
  names_.push_back(macname);
  bodies_.push_back(macbody);
  assert(nameTab_.find(macname) == nameTab_.end());
  nameTab_[macname] = nameIdx;

  return 0;
}

int MacroParser::expandMacro(unsigned idx)
{
}

int MacroParser::postProcess()
{
  for (unsigned idx = 0; idx < names_.size(); ++idx) {

    const std::string &macname = names_[idx];
    const std::string &macbody = bodies_[idx];

    WalkState state(macbody);
    bool error = false;
    while(true) {
      std::pair<MacTokenTyp, std::string> tv = getToken(state);
      switch(tv.first) {
        case TOK_ERROR:
          error = true;
          break;
        case TOK_END_OF_STRING:
          break;
        case TOK_IDENTIFIER:
          // Must be another macro
          if (nameTab_.find(tv.second) == nameTab_.end()) {
          error = true;
          break;
        }
...
int main(int argc, char **argv) {
  ...
  if (! InputObjectFile.empty()) {
    rc |= visitObjectFile(InputObjectFile, state, OS);
  }
