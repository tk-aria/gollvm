//
// This helper program emits C++ code that encapsulates information
// about available CPUs for a given architecture. It works by invoking
// clang and/or llc and inspecting trace output and generated IR.
//
...
var qurx *regexp.Regexp = regexp.MustCompile(`^"(.+)"$`)

func qutrim(s string) string {
	sl := qurx.FindSubmatch([]byte(s))
	if len(sl) == 2 {
		return string(sl[1])
	}
	return s
...
// Function definitions in an LLVM IR dump have an attribut tag (#<num>);
...
func parseClangOut(r io.Reader) (string, string) {
	...
	for scanner.Scan() {
		...
		if len(sl) == 2 {
			nmatched, serr := fmt.Sscanf(string(sl[1]), "%d", &attrnum)
		...
		if attrnum != -1 {
			...
			if len(sl) == 3 {
				var at int
				nmatched, serr := fmt.Sscanf(string(sl[1]), "%d", &at)
				if serr != nil || nmatched != 1 {
					fatal("problems matching %s in %s",
						string(sl[1]), scanner.Text())
				}
...
func enumerateAttributes(triple string, tdir string, cpus []string, bw *bufio.Writer, tf string, idx int) {
	...
	bw.WriteString(fmt.Sprintf("// triple: %s\n", triple))
	bw.WriteString(fmt.Sprintf("static const CpuAttrs attrs%d[] = {\n", idx))
