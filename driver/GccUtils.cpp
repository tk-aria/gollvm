...
static llvm::SmallVector<std::string, 4> tokenize(const std::string &s,
                                                  char delim)
{
  ...
  for (unsigned chunks = 0; chunks < 2; ++ chunks) {
    ...
    result.push_back(s.substr(cur, nxt - cur));
...
// that don't use crtbegin.o; if need be this could be switched to libgcc.a.

bool GCCInstallationDetector::validCandidate(state &s, const std::string &cand)
{
  // is there an actual installation?
  std::stringstream ss;
  ss << cand << s.suffix << "/crtbegin.o";
  std::string crtb = ss.str();
  //llvm::errs() << "=-= checking for " << crtb << "\n";
...
void GCCInstallationDetector::scanPrefix(state &s, const std::string &pref)
{
  for (auto &lib : s.libdirs) {
    ...
    for (auto &tripleAlias : s.tripleAliases) {
      ...
      scanCandidate(s, ss.str(), tripleAlias);
    }
  }
}
...
void GCCInstallationDetector::init()
{
  ...
  // If we found something viable in the sysroot, then don't look
  // any farther (this differs from clang AFAICT). The intent here is that
  // if the sysroot provides GCC X but the system has GCC X+1, we still
  // want to use the version in the sysroot.
  if (!s.version.valid()) {
