...
TEST(BackendCoreTests, TypeEquivalence) {
  ...
  Btype *s1t = mkBackendStruct(be, i8t, "f1", pft, "f2", pht, "n", nullptr);
  ...
  Btype *desct = mkBackendStruct(be, pft, "f1", nullptr);
  Btype *pdesct = be->pointer_type(desct);
  Btype *s2t = mkBackendStruct(be, i8t, "f1", pdesct, "f2", pht2, "n", nullptr);
  ...
  bool equiv = tm->fcnDescriptorCompatible(pht->type(), pht2->type(), visited);
  EXPECT_TRUE(equiv);
