...
TEST(BackendVarTests, ZeroSizedGlobals) {
  ...
  {
    const char *exp = R"RAW_RESULT(
      @emptys2f = global { { i8 }, { i8 } } zeroinitializer
    )RAW_RESULT";
  ...
  {
    const char *exp = R"RAW_RESULT(
      @emptyar = global [1 x { { i8 }, { i8 } }] zeroinitializer
    )RAW_RESULT";
  ...
  // emptyar[0] = emptys2f
  Bexpression *vex3 = be->var_expression(ga1, VE_lvalue, loc);
  Bexpression *aix = be->array_index_expression(vex3, val0, loc);
  Bexpression *vex4 = be->var_expression(gs2, VE_rvalue, loc);
  h.mkAssign(aix, vex4);

  // localemptyintar = emptyintar
  Bvariable *loc1 = h.mkLocal("localemptyintar", atint0);
  Bexpression *vex5 = be->var_expression(loc1, VE_lvalue, loc);
